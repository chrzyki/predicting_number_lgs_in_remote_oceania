...
\FloatBarrier
\subsection{Controls for spatial and phylogenetic non-indepedence} 
...
All measurements, except the response variable, were scaled and centred to make the coefficients easier to interpret and compare. Shorelines were also log-10-transformed (before scaling) to avoid an outsized effect of outliers (c.f. \citet{rolett2004environmental} and \citet{atkinson2016cultural}).
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{Predicting_lgs_DAG_slimmed}
...
\begin{itemize}
    ...
    \item WAIC - Watanabe–Akaike Cnformation Criterion. Implemented with the function waic in the R-package loo.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{brms_medium_control_spatialphylo_dropped_out_plot_diff.png}
\caption{Mean absolute difference of predicted and observed number of languages per island group (shared language island groups) including control for spatial and phylogenetic non-independence. Every column represents a new run of the model with a particular group dropped out from the observations (x-axis).}
\label{appendix_medium_control_spatialphylo_dropped_out_plot_diff}
\end{figure}
...
\input{latex/appendix_used_packages_table_versions.tex}
%\end{appendices}
