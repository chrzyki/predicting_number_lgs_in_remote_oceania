...
\centering
\includegraphics[width=19cm]{CartoGIS_oceania_languages_subgreions_edited.png}
\caption{}
...
\begin{figure}[ht]
\centering
%\includegraphics[width=13cm]{Lg_distrubition_medium_island_group_lg_merged.png}
...
%number_of_languags_vs_pop_1950_log10.png
%number_of_languags_vs_pop_1950.png
...
\centering
    ...
    \centering
    \includegraphics[width=.9\linewidth]{number_of_languags_vs_pop_1950.png}
    \caption{Population in 1950 per country (x-axis) and number of languages per country (y-axis)}\hfill
    \end{subfigure}\hfil%
    \begin{subfigure}{.45\textwidth}
    \centering
          \includegraphics[width=.9\linewidth]{number_of_languags_vs_pop_1950_log10.png}
    \caption{Population in 1950 per country (x-axis) and number of languages per country (y-axis) - log-10-transformed.}
    \end{subfigure}
    
\caption{Population per country in 1950 compared to number of languages.}
    \label{fig:pop_vs_languages}
\end{figure}
...
To test the impact of several variables predicting the same response variable we can use a BRMS model \citep{burkner2017brms}. The distribution of languages per island group is a kind of count data, and as previously discussed, it is over-dispersed (c.f. \citet[4-5]{gavin2012island}). This means we cannot use a normal distribution, as is most common. Instead, we apply a Poisson distribution, which is able to adjust the relationship between the mean and the variance appropriately. 

The formula of the full model is\footnote{Settlement order was reversed from Fig. \ref{dates_map} such that high \\ numbers represent further back in time and recent waves have a low number.}:
...
\indent \indent\texttt{PCA3\_carrying\_density  :  Shoreline (log10) +} \\
\indent \indent\texttt{Settlement order :  Shoreline (log10)} \\

%\texttt{Temperature seasonality (mean) \textasteriskcentered{}   Annual temperature (mean)  +} \\
%\texttt{Political complexity (mean) + }\\
%\texttt{Isolation (log10) + }\\ 
%\texttt{Settlement order \textasteriskcentered{}   Shoreline (log10)}\\
\end{quotation}
...
All variables were centered (mean subtracted) and then scaled to between 0 and 1 to make interpretation of coefficient distributions easier when comparing across predictors.

The results were calculated in R 3.6.0 \citep{R} using the function \texttt{brms::brm()} , tidyverse suite \citep{tidyverse13} and the modEvA-package \citep{barbosa2016package}. 

\FloatBarrier
\subsection{Results}
 
 
 Fig.~\ref{medium_model_predict} and Fig.~\ref{Marck_model_predict} illustrating the predictions the models make compared to the observed language counts. We will first discuss the shared language island groups, and secondly the overnight distance groups.
