...
If we drop one island group out at a time and re-run the model, we can asses which island groups represent the largest challenge to fitting the model. When Malakula (central Vanuatu), Kanaky (New Caledonia main island) and Santo (central Vanuatu) are left out, the difference becomes much lower (see Fig. \ref{brms_medium_dropped_out_plot_diff} in \ref{appendix_supp_figs}). They are the three island groups with the highest number of languages (see Fig \ref{polygon_plot_medium} in appendix \ref{sec:island_geo}). Political complexity remains the most important predicting factor in these three models, the coefficient does not overlap with zero.
...
For the overnight-sailing distances, several island groups are much larger than the previous grouping. Vanuatu (including Anuta and Tikopia) are all within the same group (see Fig \ref{polygon_plot_SBZR} in appendix \ref{sec:island_geo}). This also leads to a larger discrepancy between the number of languages, with Vanuatu and Temotu having 130 and S\={a}moa still only one. The absolute average difference between the predicted and observed number of languages is greater, 4.3. Fig \ref{SBZR_model_predict} shows the prediction vs observed per island group.
