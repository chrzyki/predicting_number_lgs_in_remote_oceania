...
This study tests the hypothesis that more vertical levels of political organisation is meaningfully connected to the number of languages, also when we incorporate other possible factors like settlement time, size and carrying capacity (c.f. \citet{NETTLE1998}, \citet{gavin2012island} and \citet{hua2019ecological}). If the results show that political complexity has a significant correlation with the number of languages per island group when other relevant factors are controlled for, this lends support to Turner and Pawley's hypothesis.
...
The approach of this paper is to gather data on island groups and their societies and then fit a statistical model such that we can evaluate which factors are the best at explaining the observed number of languages when they are co-estimated quantitatively. 

The unit of analysis is ``island group''. We cannot use modern political boundaries, and as we want to capture something about the communities at large we are not satisfied with using individual islands. For these reasons, we use two different ways of grouping islands: overnight sailing distances and shared language (see appendix \ref{sec:island_geo} for details).

%Predicting_lgs_DAG_full

A simple test of Spearman's Rank Order correlation reveals that the number of languages and political complexity per island group are indeed correlated (rho  = -0.4, \emph{p} value = >0.001 for islands grouped for shared language). However, this is not sufficient. We want to investigate whether this correlation still holds once we explicitly incorporate other variables that we expect might explain language diversity as well, such as time-depth and environmental factors. The data for this study consists of:
...
In order to create a formula for the statistical model, we consider the suggested causal relationships based on previous research (see section \ref{sec:previous_research}). The relationship between variables can be represented in a formalised manner in a Directed Acyclic Graph. There are set procedures for how to prune and translate a DAG for modelling (\citet{pearl1995causal} and \citet{mcelreath2020statistical}). 

The variables Net Primary Production (from NASA satellites Terra and Aqua resp.), temperature \& rainfall (mean and seasonality) and absolute latitude are all related to each other 


Fig. \ref{Predicting_lgs_DAG_slimmed} shows the smaller DAG that represents the final model formula. For the full DAG, including excluded variables and variables with missing data, see appendix \ref{appendix_DAG_def}.
...
\begin{figure}[ht]
\centering
\includegraphics[width=15cm]{latex/SLOM_SBZR_all_variables.png}
\caption{Scatterplot matrix of all variables available, over overnight-sailing-groups.}
\label{SLOM_SBZR_all_variables}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=15cm]{latex/SLOM_medium_all_variables.png}
\caption{Scatterplot matrix of all variables available, over shared-language-groups.}
\label{SLOM_medium_all_variables}
\end{figure}


\begin{figure}[ht]
\centering
    \includegraphics[width=15cm]{latex/Map_RO_dates.png}
\caption{Scatterplot matrix of all variables available, over shared-language-groups.}
\label{SLOM_medium_all_variables}
\end{figure}
