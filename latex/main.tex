...
The variables Net Primary Production (NPP; the amount of biomass or carbon produced), temperature \& rainfall (mean and seasonality) and absolute latitude are all associated with the suitability of a place for life adn correlated with each other (c.f. Figs \ref{SPLOM_medium_all_variables } and \ref{SPLOM_SBZR_all_variables}). A more fertile environment for plants, animals and humans is generally found in places with higher and stable temperature, higher amounts of biomass etc. They are 

It is not possible to include historic population data, because it does not exist for too many places, but 

\ref{SPLOM_country_all_variables}
...
This resulted in the following model formula\footnote{Settlement order was reversed from Fig. \ref{dates_map} such that high numbers represent further back in time.}:
